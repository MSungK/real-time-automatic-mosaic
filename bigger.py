...
################################# ADD FACE RECOGNITION #################################
from car.models.experimental import car_attempt_load
from car.utils.datasets import LoadStreams, LoadImages
from car.utils.general import car_check_img_size, car_check_requirements, car_check_imshow, car_non_max_suppression, car_apply_classifier, \
    car_scale_coords, car_xyxy2xywh, car_strip_optimizer, car_set_logging, car_increment_path
from car.utils.plots import car_plot_one_box
from car.utils.torch_utils import car_select_device, car_load_classifier, car_time_synchronized, TracedModel
...
def detect(opt):
    ...
    # Load model
    ################################# CAR RECOGNITION #################################
    car_model = car_attempt_load('car/epoch_224.pt', map_location=device)
    ################################# CAR RECOGNITION #################################
    ...
    if device.type != 'cpu':
        ...
        ################################# CAR RECOGNITION #################################
        car_model(torch.zeros(1, 3, imgsz, imgsz).to(device).type_as(next(car_model.parameters())))  # run once
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        if device.type != 'cpu' and (old_img_b != img.shape[0] or old_img_h != img.shape[2] or old_img_w != img.shape[3]):
            ...
            for i in range(3):
                car_model(img, augment=opt.augment)[0]
        ...
        ################################# CAR RECOGNITION #################################
        with torch.no_grad():
            car_pred = car_model(img, augment=opt.augment)[0]
        ################################# CAR RECOGNITION #################################
        print(pred[...,4].max())
        # Apply NMS
        ################################# CAR RECOGNITION #################################
        car_pred = non_max_suppression(car_pred, 0.5, 0.45, 0, agnostic=opt.agnostic_nms)
        ################################# CAR RECOGNITION #################################
       ...
       # Process detections
        for i, det in enumerate(car_pred):  # detections per image
    ...
    for i in plot_car:
        plot_one_box(i, im0, label=label, mosaic=True, show_box=show_box, color=colors(c, True), line_thickness=opt.line_thickness,  steps=3, orig_shape=im0.shape[:2])
